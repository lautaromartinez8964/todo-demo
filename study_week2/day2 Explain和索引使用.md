...
3.  **变长字段长度标识（针对 `VARCHAR`）**：
    *   如果最大字段长度<255, `VARCHAR` 需要额外的 **1 字节** 存储实际字符串长度。如果最大字段长度>255, `VARCHAR` 需要额外的 **2 字节** 存储实际字符串长度。
4.  **NULL 值标识**：
...
$$\text{key\_len} = (50 \times 4) + 1 (\text{变长长度}) + 1 (\text{NULL 标记}) = \mathbf{202 \text{ 字节}}$$

如果定义为 `scene_id VARCHAR(50) NOT NULL`，则 `key_len` 为 $200 + 2 = \mathbf{201 \text{ 字节}}$。
...
*   **八进制数值：176**
**计算公式与详细推导：**
在 MySQL 中，`key_len` 表示查询用到的索引长度，计算规则主要包含三个方面：**字段定义长度**、**字符集开销**、以及**是否为空/变长开销**。
1.  **计算 `age` 字段 (INT NOT NULL)：**
    *   **字段类型**：INT 类型固定占用 **4 字节**。
    *   **NULL 属性**：定义为 `NOT NULL`，因此不需要 NULL 标识位（1字节）。
    *   **变长属性**：INT 是定长类型，不需要长度前缀位。
    *   **小计**：4 字节。
2.  **计算 `name` 字段 (VARCHAR(30) DEFAULT NULL, utf8mb4)：**
    *   **字段类型与字符集**：`VARCHAR(30)` 且字符集为 `utf8mb4`。`utf8mb4` 字符集最多使用 4 个字节表示一个字符。
    *   **最大存储长度**：30 字符 * 4 字节/字符 = **120 字节**。
    *   **变长字段长度前缀**：VARCHAR 是变长字段，需要额外的空间记录实际长度。
        *   规则：如果字段最大长度 <= 255 字节，占用 1 字节；如果 > 255 字节，占用 2 字节。
        *   此处最大 120 字节 (< 255)，故占用 **1 字节**。
    *   **NULL 属性**：定义为 `DEFAULT NULL`（可为空），InnoDB 需要额外的 **1 字节** 来标识该字段是否为 NULL。
    *   **小计**：120 (数据) + 1 (长度前缀) + 1 (NULL标识) = **122 字节**。
3.  **总 key_len 计算：**
    *   总计 = `age` 长度 + `name` 长度
    *   总计 = 4 + 122 = **126 字节**。
---
### 二、 索引失效与物理分析
**问题 1：Extra 字段里出现了什么？**
*   **答案**：在 MySQL 5.6 及以上版本中，Extra 字段通常会出现 **`Using index condition`** (索引下推)。
    *   如果是旧版本 MySQL (5.5 及以下) 或者未开启 ICP 优化，则会显示 **`Using where`**。
**问题 2：结合 B+ 树的字典序，解释为何列 a 用到了索引，而列 c 无法做二分查找？**
*   **核心原因**：复合索引遵循“最左前缀原则”，B+ 树的排序规则是“先按 a 排序，在 a 相同的情况下再按 b 排序...”。
*   **详细解释**：
    1.  **字典序原理**：复合索引 `KEY (a, b, c)` 构建的 B+ 树，其节点中的键值是按照 `(a, b, c)` 的顺序整体排序的。这就像一本字典，先按“拼音首字母”排序，首字母相同的再按“笔画”排序。
    2.  **列 a 为何有效**：当查询条件是 `WHERE a = 1` 时，由于 B+ 树首先就是按 `a` 排序的，所有 `a=1` 的记录在物理存储上是连续在一起的。因此，数据库可以直接通过二分查找定位到 `a=1` 的起始位置，并顺序扫描。
    3.  **列 c 为何失效（指二分查找失效）**：当查询条件是 `WHERE a = 1 AND c = 'alpha'` 时，由于缺少了中间的列 `b`，打破了排序的连续性。
        *   在 `a=1` 的范围内，数据是按 `b` 排序的，而不是按 `c` 排序。
        *   例如，数据可能是：`(a=1, b=1, c='z')`, `(a=1, b=2, c='a')`, `(a=1, b=3, c='m')`。
        *   可以看到，虽然 `a` 是有序的，但 `c` 在 `a=1` 的范围内是无序的（'z', 'a', 'm'）。既然 `c` 是无序的，就无法使用二分查找快速定位 `c='alpha'`。
    4.  **结论**：跳过中间列 `b` 直接查 `c`，导致 `c` 失去了索引的有序性支持，因此只能通过“扫描”而非“查找”来匹配 `c`。
---
### 三、 Linux 信号题
**问题 1：第一步应该发送什么信号？为什么不能直接使用 kill -9？**
*   **命令**：`kill -15 <PID>` (或者直接 `kill <PID>`，默认即为 -15)。
*   **原因分析**：
    *   **优雅停止**：`kill -15` 发送的是 **SIGTERM** 信号。这是一个“温和”的请求信号，进程收到后，有机会执行清理工作（如完成正在进行的迁移任务、释放锁、关闭文件句柄、断开数据库连接），然后安全退出。
    *   **强制终止的风险**：`kill -9` 发送的是 **SIGKILL** 信号。这是一个“暴力”的强制终止信号，操作系统内核立即强制杀死进程。
        *   **数据安全**：对于数据迁移进程，强制杀掉可能导致数据只写了一半（脏写）、数据库连接未正常关闭导致锁未释放、或者迁移状态未保存，下次重启时造成数据错误或重复迁移。
        *   **资源泄漏**：可能导致临时文件未被清理，或共享内存段残留。
**问题 2：RSS 和 VSZ 哪一个代表物理内存真实占用？**
*   **答案**：**RSS (Resident Set Size)**。
*   **解析**：
    *   **RSS**：代表驻留集大小，即进程当前**实际驻留在物理内存（RAM）中**的内存总和。它不包含已换出到磁盘（Swap）的内存，是衡量进程物理内存占用的真实指标。
    *   **VSZ (VSZ/VIRT)**：代表虚拟内存大小。包含了进程申请的所有内存（包括未实际分配物理页面的、已换出到 Swap 的、以及映射到的共享库文件）。VSZ 通常远大于实际物理内存占用，不能真实反映物理内存压力。
